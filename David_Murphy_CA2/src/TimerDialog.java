...
public class TimerDialog extends JDialog  {
	...
	private void initComponents() {
		...
		// TODO: This start action listener will be invoked when the start button is clicked.
				// It should take the values from the three text fields and try to convert them into integer values,
				//and then check for NumberFormatExceptions 
	...
	if(mins<0||mins>59) {
			JOptionPane.showMessageDialog(displayPanel,"Minutes value must be between 0 and 59 ",
					"Error Title",JOptionPane.ERROR_MESSAGE);
			valid=false;
			
		}
		if(secs<0||secs>59) {
			JOptionPane.showMessageDialog(displayPanel,"Seconds value must be between 0 and 59 ",
					"Error Title",JOptionPane.ERROR_MESSAGE);
			valid=false;
		}
